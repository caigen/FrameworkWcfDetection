...
To scan a repository, select the `find-framework-wcf-projects` agent or invoke its namespaced identifier from Copilot CLI. The agent deterministically discovers projects with its bundled PowerShell script, then uses a separate subagent to check each project:
